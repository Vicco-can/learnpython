# 1、 try to change the URL address.
# 看提示图片中为2**38，计算值为274877906944。
print(2**38)


# 2、Hint 1：K->M  O->Q  E->G
#
# 此提示可看出是位移计算，都是位移两位
#
...
#
# 想必是利用Hint 1的唯一算法转换字符串，得到下一关的地址信息，
# 需要注意的是转换是在26个字母中转换的
def change_str(s):
    if s == 'y':
        return 'a'
        ...
        return 'b'
    elif s.isalpha(): # isalpha() 方法检测字符串是否只由字母组成
        return chr(ord(s)+2)
    else:
        return s
...
res = map(change_str, s)
print(''.join(map(change_str, 'map')))
print(''.join(res))


# 3、根据提示密码藏在图片的书中，或者页面源码中，看了下图片果断F12，
# 看到页面源码中有一大段字符串，提示找出重复最少的
s3='''
%%$@_$^__#)^)&!_+]!*@&^}@[@%]()%+$&[(_@%+%$*^@$^!+]!&_#)_*}{}}!}_]$[%}@[{_@#_^{*
@##&{#&{&)*%(]{{([*}@[@&]+!!*{)!}{%+{))])[!^})+)$]#{*+^((@^@}$[**$&^{$!@#$%)!@(&
+^!{%_$&@^!}$_${)$_#)!({@!)(^}!*^&!$%_&&}&_#&@{)]{+)%*{&*%*&@%$+]!*__(#!*){%&@++
...
# 4、Hint ：One small letter, surrounded by EXACTLY three big bodyguards on each of its sides.
#
...
def find_letter(s):
    ...
    for i in range(4, len(s)-4):
        if s[i-4].islower() and s[i-3].isupper() and s[i-2].isupper() and s[i-1].isupper() \
                and s[i].islower() and s[i+1].isupper() and s[i+2].isupper() and s[i+3].isupper() and s[i+4].islower():
            res+=s[i]
            # print(s[i])
    # print(res)
    return res

print(find_letter(s4))

# 5、<!-- urllib may help. DON'T TRY ALL NOTHINGS, since it will never
